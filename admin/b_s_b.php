...
if(!empty($_REQUEST['id_fp']))
	...
	// 	    }
    //         $uploadDirectory = "image/bus_str_banner/";
	  ...
	  $e_ban_content =$_POST['e_ban_content'];
        ...
        // }
             ...
             if(!empty($e_ban_title) && !empty($e_ban_content))
                {
                    $insq1 = mysqli_query($dbcon1,"UPDATE `bus_str_banner` SET `ban_title`='".$e_ban_title."',`ban_content`='".$e_ban_content."' WHERE `id_str_banner`=".$id_fp."") or die(mysqli_Error($dbcon1));
                    if($insq1)
                    {
 ...
 <?php include 'menu.php';?>
	...
	<div class="main-container">
		<div class="pd-ltr-20 xs-pd-20-10">
			<div class="min-height-200px">
				<div class="page-header">
					<div class="row">
						...
						<div class="col-md-6 col-sm-12">
							<div class="title">
								<h4> Edit Business Strategy Banner </h4>
							</div>
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
									<li class="breadcrumb-item"><a href="index.php">Home</a></li>
									<li class="breadcrumb-item active" aria-current="page"><a href="b_s_b.php">Business Strategy Banner</a></li>
								</ol>
							</nav>
						</div>
					</div>
				...
				<?php
       				 $query ="SELECT `id_str_banner`,`ban_title`, `ban_content`, `date_time`, `active_record` FROM `bus_str_banner` WHERE   id_str_banner=".$id_fp."";
        			$execute_qry = mysqli_query($dbcon1,$query)or die("Banner Retriew Error!");          
       	 			while($team = mysqli_fetch_array($execute_qry)) {
        			$id_str_banner = $team['id_str_banner'];
	...
	// active status start
      ...
      if(!empty($id_fp))
      {
        ...
        if ($active_record == 0)
        {
          $insq1 = mysqli_query($dbcon1,"UPDATE `bus_str_banner` SET `active_record`=1 WHERE `id_str_banner`=".$id_fp."") or die(mysqli_Error($dbcon1));
          if($insq1)
          {
            echo('<script type="text/javascript">alert(" Activated Sucessfully"); window.location="b_s_b.php";</script>');
           // $post_msg = "client Delete successfully";
          }
          else
          {
            echo('<script type="text/javascript">alert(" Activation Problems"); window.location="b_s_b.php";</script>');
            //$err_msg = "client Delete Problems";
          }
          }
        else
        {
            $insq1 = mysqli_query($dbcon1,"UPDATE bus_str_banner SET active_record = 0  WHERE `id_str_banner`=".$id_fp."") or die(mysqli_Error($dbcon1));
          if($insq1)
          {
            echo('<script type="text/javascript">alert(" UnActivated Sucessfully"); window.location="b_s_b.php";</script>');
           // $post_msg = "client Delete successfully";
          }
          else
          {
            echo('<script type="text/javascript">alert("UnActivated Problems"); window.location="b_s_b.php";</script>');
            //$err_msg = "client Delete Problems";
          }
        }
        }
	...
	// Delete start
      ...
      if(!empty($id_fp))
      {
            ...
            $insq1 = mysqli_query($dbcon1,"DELETE FROM `bus_str_banner` WHERE `id_str_banner`=".$id_fp."") or die(mysqli_Error($dbcon1));
          if($insq1)
          {
            echo('<script type="text/javascript">alert(" Banner  Delete Sucessfully"); window.location="b_s_b.php";</script>');
           // $post_msg = "client Delete successfully";
          }
          else
          {
            echo('<script type="text/javascript">alert(" Banner  Delete Problems"); window.location="b_s_b.php";</script>');
            //$err_msg = "client Delete Problems";
          }
      }
    } 
...
<?php
    ...
    if(isset($_POST['ban_title']))
     ...
     if(!empty($ban_title) && !empty($ban_content))
		{
			$selq1 = mysqli_query($dbcon1,"SELECT * FROM bus_str_banner WHERE ban_title='$ban_title' and active_record=1") or die(mysqli_Error($dbcon1));
			...
			{
				$insq1 = mysqli_query($dbcon1,"insert into bus_str_banner(ban_title,ban_content) values('$ban_title','$ban_content')") or die(mysqli_Error($dbcon1));
			if($insq1)
			{
...
<?php include 'menu.php';?>
	...
	<div class="main-container">
			...
			<div class="min-height-200px">
				<div class="page-header">
					<div class="row">
						<div class="col-md-6 col-sm-12">
							<div class="title">
								<h4>Business Strategy Banner</h4>
							</div>
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
									<li class="breadcrumb-item"><a href="index.php">Home</a></li>
        ...
        <!-- Data tables strt -->
				...
				<?php
                   $query = "SELECT `id_str_banner`,`ban_title`, `ban_content`, `date_time`, `active_record` FROM `bus_str_banner`";
                   $execute_qry = mysqli_query($dbcon1,$query)or die("Banner Retriew Error!");          
                   $i=0;
				   while($team = mysqli_fetch_array($execute_qry)) {
				   $i++;
                ...
                ?>
							<tr>
								...
								<td><a href='?upd=<?php print $id_str_banner ?>'><?php if($active_record == 0) { ?><span class="badge badge-danger">InActive</span><?php } else{ ?><span class="badge badge-success">Active</span><?php } ?></a>&nbsp; 
									<a href='?id_fp=<?php print $id_str_banner ?>'><input type='hidden' name='po' value="<?php print $id_str_banner ?>"/><span class="badge badge-warning">Edit</span></a>&nbsp;
									<a href='?del=<?php print $id_str_banner ?>'><input type='hidden' name='dl' value="<?php print $id_str_banner ?>"/> <span class="badge badge-danger">Delete</span></a>
							   </td>
							</tr>
				<?php
                  }
