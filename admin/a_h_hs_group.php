    ...
    { 
      ...
      else
      {
        $e_hs_group_img_path = $existing_image;
				...
				$data = getimagesize($e_hs_group_img_path);
                $width = $data[0];
                $height = $data[1];
              ...
              else if(!empty($e_hs_group_title) && !empty($e_hs_group_content))
                {
                    ...
                    if($insq1)
                    {
                    $post_msg = "Your About HS Group Edited successfully";
                    }
                    else
                    {
                    $err_msg = "Your About HS Group Editing Problems";
                    }
                }
                else
                {
                $err_msg = "You must be provide a valid About HS Group Image and About HS Group Title and About HS Group Content";
                }	
    }?>
 ...
 <?php include 'menu.php';?>
		...
		<div class="pd-ltr-20 xs-pd-20-10">
			<div class="min-height-200px">
						...
						<div class="col-md-6 col-sm-12">
							...
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
									...
									<li class="breadcrumb-item active" aria-current="page"><a href="a_h_hs_group.php">Heritage HS Group</a></li>
								</ol>
							</nav>
						</div>
					</div>
				...
				<?php
        			...
        			$execute_qry = mysqli_query($dbcon1,$query)or die("About HS Group Retriew Error!");          
	...
	// active status start
      ...
      if(!empty($id_fp))
      {
        ...
        if ($active_record == 0)
        {
          ...
          if($insq1)
          {
            echo('<script type="text/javascript">alert(" Activated Sucessfully"); window.location="a_h_hs_group.php";</script>');
           // $post_msg = "client Delete successfully";
          }
          else
          {
            echo('<script type="text/javascript">alert(" Activation Problems"); window.location="a_h_hs_group.php";</script>');
        ...
        else
        {
          ...
          if($insq1)
          {
            echo('<script type="text/javascript">alert(" UnActivated Sucessfully"); window.location="a_h_hs_group.php";</script>');
           // $post_msg = "client Delete successfully";
          }
          else
          {
            echo('<script type="text/javascript">alert("UnActivated Problems"); window.location="a_h_hs_group.php";</script>');
            //$err_msg = "client Delete Problems";
          }
        }
        }
	...
	// Delete start
      ...
      if(!empty($id_fp))
      {
          ...
          if($insq1)
          {
            echo('<script type="text/javascript">alert(" About HS Group  Delete Sucessfully"); window.location="a_h_hs_group.php";</script>');
           // $post_msg = "client Delete successfully";
          }
          else
          {
            echo('<script type="text/javascript">alert(" About HS Group  Delete Problems"); window.location="a_h_hs_group.php";</script>');
            //$err_msg = "client Delete Problems";
          }
      }
    } 
    ...
    if(isset($_POST['hs_group_title']))
       { 
        ...
        $hs_group_content =$_POST['hs_group_content'];
		       ...
		       $data = getimagesize($hs_group_img_path);
                $width = $data[0];
                $height = $data[1];
                if($height != "450" && $width !="650"){
                $err_msg = 'Your Given Height('.$height.'),Width('.$width.') Is Not Matching';
       ...
       else if(!empty($hs_group_title) && !empty($hs_group_content) && !empty($hs_group_img_path))
		{
			$selq1 = mysqli_query($dbcon1,"SELECT * FROM ab_hs_group WHERE  active_record=1") or die(mysqli_Error($dbcon1));

			if (mysqli_num_rows($selq1) > 0) 
			{
				//echo "sorry"; exit;
			$url_Error = "Sorry...  HS Group  Already Exist";
			}
			else
			{//echo $hs_group_title."</br>".$hs_group_content."</br>".$hs_group_img_path; exit;
				$insq11 = mysqli_query($dbcon1,"INSERT INTO `ab_hs_group`(`hs_group_img_path`, `hs_group_title`, `hs_group_content`) VALUES ('".$hs_group_img_path."' ,'".$hs_group_title."','".$hs_group_content."')") or die(mysqli_Error($dbcon1));
			if($insq11)
			{//echo "sucess"; exit;
				$post_msg = "Your About HS Group Added successfully";
			}
			else
			{
				//echo "sorry"; exit;
				$err_msg = "Your About HS Group adding Problems";
			}
		}
