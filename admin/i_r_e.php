    ...
    { 
      ...
      if($_FILES["e_uploadfi"]["size"]>0)
	    {
            ...
            $uploadDirectory = "image/E-Disclosure_pdf/";
      ...
      if($_FILES["e_uploadfile"]["size"]>0)
	    {
            ...
            $uploadDirectory = "image/ir_e_img/";
	  ...
	  if($height != "768" && $width !="960"){
		$err_msg = 'Your Given Height('.$height.'),Width('.$width.') Is Not Matching';
                ...
                {
                    $insq1 = mysqli_query($dbcon1,"UPDATE `ir_e` SET `ir_title`='".$e_ir_title."',`ir_content`='".$e_ir_content."',`ir_img_path`='".$e_ir_img_path."' WHERE `id_ir`=".$id_fp."") or die(mysqli_Error($dbcon1));
                    if($insq1)
                    {
                    $post_msg = "Your Investor Relations - E-Disclosure Edited Successfully";
                    }
                    else
                    {
                    $err_msg = "Your Investor Relations - E-Disclosure Editing Problems";
                    }
                }
                else
                {
                $err_msg = "You Must Be Provide a Valid Investor Relations - E-Disclosure Details";
                }	
    }?>
 ...
 <?php include 'menu.php';?>
	...
	<div class="main-container">
		<div class="pd-ltr-20 xs-pd-20-10">
			<div class="min-height-200px">
				<div class="page-header">
					<div class="row">
						...
						<div class="col-md-6 col-sm-12">
							<div class="title">
								<h4> Edit Investor Relations - E-Disclosure</h4>
							</div>
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
									<li class="breadcrumb-item"><a href="index.php">Home</a></li>
									<li class="breadcrumb-item active" aria-current="page"><a href="i_r_e.php">Investor Relations</a></li>
								</ol>
							</nav>
						</div>
					</div>
				...
				<?php
       				 $query ="SELECT `id_ir`, `ir_img_path`, `ir_title`, `ir_content`, `pdf_path`, `date_time`, `active_record` FROM `ir_e` WHERE   id_ir=".$id_fp."";
        			$execute_qry = mysqli_query($dbcon1,$query)or die("Investor Relations - E-Disclosure Retriew Error!");          
	...
	// active status start
      ...
      if(!empty($id_fp))
      {
        ...
        if ($active_record == 0)
        {
          $insq1 = mysqli_query($dbcon1,"UPDATE `ir_e` SET `active_record`=1 WHERE `id_ir`=".$id_fp."") or die(mysqli_Error($dbcon1));
          if($insq1)
          {
            echo('<script type="text/javascript">alert(" Activated Sucessfully"); window.location="i_r_e.php";</script>');
           // $post_msg = "client Delete successfully";
          }
          else
          {
            echo('<script type="text/javascript">alert(" Activation Problems"); window.location="i_r_e.php";</script>');
            //$err_msg = "client Delete Problems";
          }
          }
        else
        {
            $insq1 = mysqli_query($dbcon1,"UPDATE ir_e SET active_record = 0  WHERE `id_ir`=".$id_fp."") or die(mysqli_Error($dbcon1));
          if($insq1)
          {
            echo('<script type="text/javascript">alert(" UnActivated Sucessfully"); window.location="i_r_e.php";</script>');
           // $post_msg = "client Delete successfully";
          }
          else
          {
            echo('<script type="text/javascript">alert("UnActivated Problems"); window.location="i_r_e.php";</script>');
            //$err_msg = "client Delete Problems";
          }
        }
        }
	...
	// Delete start
      ...
      if(!empty($id_fp))
      {
            ...
            $insq1 = mysqli_query($dbcon1,"DELETE FROM `ir_e` WHERE `id_ir`=".$id_fp."") or die(mysqli_Error($dbcon1));
          if($insq1)
          {
            echo('<script type="text/javascript">alert(" Investor Relations - E-Disclosure  Delete Sucessfully"); window.location="i_r_e.php";</script>');
           // $post_msg = "client Delete successfully";
          }
          else
          {
            echo('<script type="text/javascript">alert(" Investor Relations - E-Disclosure  Delete Problems"); window.location="i_r_e.php";</script>');
            //$err_msg = "client Delete Problems";
          }
      }
    } 
...
<?php
    ...
    if(isset($_POST['ir_title']))
       { 
		...
		$uploadDirectorys = "image/E-Disclosure_pdf/";
    ...
    if (($_FILES["uploadfi"]["size"] < 90000000000000000000000000000000000) && in_array($file_types, $allowedExtss))
		...
		{
			$selq1 = mysqli_query($dbcon1,"SELECT * FROM ir_e WHERE  active_record=1") or die(mysqli_Error($dbcon1));

			if (mysqli_num_rows($selq1) > 0) 
			{
			$url_Error = "Sorry... Investor Relations - E-Disclosure  Already Exist";
			}
			else
			{
				$insq1 = mysqli_query($dbcon1,"insert into ir_e(ir_title,ir_content,ir_img_path,pdf_path) values('$ir_title','$ir_content', '$ir_img_path','$pdf_path')") or die(mysqli_Error($dbcon1));
			if($insq1)
			{
				$post_msg = "Your Investor Relations - E-Disclosure Added Successfully";
			}
			else
			{
				$err_msg = "Your Investor Relations - E-Disclosure Adding Problems";
			}
		}
			...
			{
				$err_msg = "You Must Be Provide a Valid Investor Relations - E-Disclosure ";
			}		
	}
?>
			...
			<div class="min-height-200px">
				<div class="page-header">
					<div class="row">
						<div class="col-md-6 col-sm-12">
							<div class="title">
								<h4>Investor Relations - E-Disclosure</h4>
							</div>
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
									<li class="breadcrumb-item"><a href="index.php">Home</a></li>
        ...
        <!-- Data tables strt -->
				...
				<div class="pd-20 card-box mb-30">
					<div class="clearfix mb-20">
						<div class="pull-left">
							<h4 class="text-blue h4"> Investor Relations E-Disclosure List</h4>
						</div>
					</div>
				...
				<?php
                   $query = "SELECT `id_ir`, `ir_img_path`, `ir_title`, `ir_content`, `pdf_path`, `date_time`, `active_record` FROM `ir_e`";
                   $execute_qry = mysqli_query($dbcon1,$query)or die("Investor Relations - E-Disclosure Retriew Error!");          
                   $i=0;
				   while($team = mysqli_fetch_array($execute_qry)) {
				   $i++;
